...
#include "SrtBase.h"
#include <array>
#define pairVec vector<pair<double, string>>
...
protected:
	...
	pairVec vec;
public:
	virtual void ReadFile(istream& input) = 0;
	...
	virtual pairVec Arrange()
	{
		...
		vec.erase(remove_if(vec.begin(), vec.end(), [](pair<double, string> p) {return p.second == "delete"; }), vec.end());
	...
	virtual string WriteToFile(string filename) = 0;
};


bool TimeLineCheck(int i, char c)
{
	array<int,18> Arr = { 1,2,4,5,7,8,10,11,12,16,17,19,20,22,23,25,26,27 };
